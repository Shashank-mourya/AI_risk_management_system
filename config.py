"""
Shared constants. Import from here; don't redefine them locally.

MIN_GAP_DAYS and RETURN_WINDOW_DAYS were once declared independently in six
files, one of them under a "must match build_labels.py" comment - which is a
comment doing a constant's job. Same story with the cost model and the scoring
path. A label horizon that drifts between the labeller, the feature builder and
the database quietly changes what "returned" means depending on which script you
ask.

The label: an order is returned if at least one of its lines is reversed by a
credit note raised between MIN_GAP_DAYS and RETURN_WINDOW_DAYS after purchase.
Both bounds came out of the data.

Lower bound - 11.1% of matched credit notes land on the purchase date itself.
Those are clerical corrections; without the floor the model learns to predict
the retailer's own data-entry errors.

Upper bound - maturity guarantees every labelled order 90 days of observation,
but the label used to count returns at any horizon, so a Dec 2009 order was
watched for 667 days and a Sep 2011 one for 90. The positive rate then tracks
the length of the watch (17.6% in the shortest-window quintile against 20.3% in
the longest), which is exposure time, not risk - and a chronological split turns
that into a train/test gap out of nothing.
"""
