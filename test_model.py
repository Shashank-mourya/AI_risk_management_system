"""
Runs the saved artefacts against the held-out split. Run after the training
notebook has written artefacts/.

    python test_model.py

Invariants, not accuracy - every check is a property that must hold whatever the
data is, so the same suite covers the real features.pkl and the synthetic
fallback. The data source follows the artefacts: whatever threshold.json says it
was trained on is what gets scored. Nothing re-trains anything.
...
It says nothing about whether the model is any good. If threshold.json reports
DATA_IS_SYNTHETIC, the precision and recall reproduced here measure the
generator rather than the model, and the summary says so.
"""
...
def main():
    ...
    if meta.get("DATA_IS_SYNTHETIC"):
        ...
        print("  *** No metric here is reportable. See make_synthetic_dataset.py. ***")
